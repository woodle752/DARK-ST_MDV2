...
// Commande tagall - Taguer tout le monde dans le groupe
function tagall(client, message) {
    const groupId = message.key.remoteJid;
    client.groupMetadata(groupId).then((metadata) => {
        const participants = metadata.participants.map(p => p.id);
        const mentions = participants.join('@c.us ') + '@c.us';
        client.sendMessage(groupId, mentions, { contextInfo: { mentionedJid: participants } });
    });
}

// Commande spu - Supprimer tout le monde sauf toi-même
function spu(client, message) {
    const groupId = message.key.remoteJid;
    const senderId = message.participant;
    
    client.groupMetadata(groupId).then((metadata) => {
        const participants = metadata.participants.map(p => p.id);
        const toKick = participants.filter(id => id !== senderId);

        toKick.forEach(member => {
            client.groupParticipantsUpdate(groupId, [member], "remove");
        });
    });
}
...
module.exports = {
  ...
  // Ajout des nouvelles fonctions ici
  tagall: tagall,
  spu: spu,
};
